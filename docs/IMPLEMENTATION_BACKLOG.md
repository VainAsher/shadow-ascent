...
- [x] Extract `HudRenderer` from `UISubsystem` (2026-05-09): all mutable HUD state + `drawHud()` body moved to `HudRenderer`; `UISubsystem` delegates via `hudRenderer.draw(g, state)` + `hudRenderer.tick(x, y, dt)`; ability-unlock events routed back through `Consumer<String> eventLogSink`; 53/53 pass.
- [x] Extract `StoryManager` from `PlaytestClient` (2026-05-09): `seedState()`, `refreshFromStoryChange()`, `restoreAbilityTriggers()`, `restoreCombatEncounters()` extracted; all 4 inline hub+mission update pairs replaced; 53/53 pass.
- [x] Extract `MissionUiCoordinator` from `PlaytestClient` (2026-05-09): objective-advance + feedback-flash chain; `feedbackSink`/`logSink` consumers; 3 PlaytestClient methods are 1-liner delegates; 53/53 pass.
...
- [x] **M4 Authored Act I Plateau Content — SUMMIT_SHRINE** (2026-05-09): 5 critical narrative beats (`beat_empty_hub_only_maiden` → `beat_aen_hollowed`); NPC registry (YIN, YANG, VEIL_MAIDEN, SIREN_OF_MASKS); `plateau_quest_policy` (critical_only_no_side_quests, `quest_chains_permitted: false`); 3 elastic chunk templates; chunk grammar tags (6 rules); story flags (`mask_truth_seen`, `siren_fight_started`, `yin_yang_taken`, `hollowed`, `act2_unlocked`); gate: beats=45, critical_flags=69, validation issues: none.
- [x] **M4 Authored Act II Plateau Content — HOLLOW_DEPTHS** (2026-05-09): 4 side quest chains (SHADE_HERMIT, SMITH_MONK, LISTENING_ELDER, ADVOCATE) in `quests.json`; 9 `hd_*_complete` flags in `story_flags.json`; NPC registry + dialogue entries authored; 10 critical-route beats cover full Act 2 arc (route_order 200–290); all quest gate flags verified against beat `sets_flags` — `awoke_in_depths`, `hollow_weight_understood`, `weightbound_ogre_defeated`, `dash_restored`, `echoes_encountered`, `double_jump_restored`, `stone_judge_defeated` all set by existing beats; gate: passes clean.
